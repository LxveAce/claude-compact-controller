...
// Sweep orphaned atomic-write temp files (`.<name>.<pid>.<ms>.tmp`). A CAUGHT write failure already
// unlinks its own temp, so an orphan only survives a HARD crash/kill between the temp write and the
// rename. Selection paths filter to `vault-*.json` so these are harmless junk, but they accumulate on
// disk. Only temps older than `maxAgeMs` are swept, so a concurrent in-flight write's fresh temp
// (milliseconds old) is never removed. Best-effort: never throws.
function cleanupStaleTmp(dir, maxAgeMs = 3600000) {
    let entries;
